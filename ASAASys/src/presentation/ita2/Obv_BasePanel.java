...
import presentation.ita2.Atr_BasePanel.Chart_ImgPanel;
...
public class Obv_BasePanel extends JPanel {
	 ...
	 class Chart_ImgPanel extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			for(int i=0;i<list1.size()-1;i++){
				ObvVO temp1=list1.get(i);
				ObvVO temp2=list1.get(i+1);
				double Shoushu1=Double.parseDouble(temp1.getObv());
				double Shoushu2=Double.parseDouble(temp2.getObv());
								
				int x=(int)(1+i*18);
				int y=(int)(515-(Shoushu2-datamin)*perXiangSu);
				
				float lineWidth = 2.0f;
				
				if(Shoushu1<=Shoushu2)
...
//				((Graphics2D)g).setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
			      ...
			      ((Graphics2D)g).fillRect(x, y, 18, 530-y);
	 ...
	 class Chart_LocPanel extends JPanel{
		 protected void paintComponent(Graphics g) {
				super.paintComponent(g);//保留该面板上的其他组件
					...
					g.drawLine(0, yLoc1, 921, yLoc1);
					g.drawLine(xLoc+9, 0, xLoc+9, 530);
		 ...
		 Chart_LocPanel(){
				...
				public void mouseClicked(MouseEvent e) {
					...
					ObvVO temp1=list1.get(selected+1);
					double data1=Double.parseDouble(temp1.getObv());
						...
						Obv_BasePanel.this.setInfo(selected);
	...
	public Obv_BasePanel() {
       ...
       JLabel lblbias = new JLabel("能量潮(Obv)");
	...
	public void DataInit(String numOfStock,String date){
		...
		try {
			list1=bl.getObv(numOfStock, 51, date);
		...
		for(int i=1;i<51;i++){
			ObvVO tempVO1=list1.get(i);
			
			double Data1=Double.parseDouble(tempVO1.getObv());
	...
	public void labelInit(){
		...
		lblNewLabel_1.setText(list1.get(10).getday());
	...
	void setInfo(int select){
		...
		ObvVO tempVO1=list1.get(select+1);
...
//		String Feestr = df.format(Fee);
		...
		LB1.setText("Obv："+tempVO1.getObv());
