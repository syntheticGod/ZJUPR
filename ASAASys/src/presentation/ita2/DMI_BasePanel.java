...
public class DMI_BasePanel extends JPanel {
	...
	int yLoc4;
	 ...
	 protected void paintComponent(Graphics g) {
			...
			((Graphics2D)g).setColor(Color.cyan);
		      ...
		      ((Graphics2D)g).drawLine(575, 40, 605, 40);
		      ((Graphics2D)g).setColor(Color.white);
		      ((Graphics2D)g).drawLine(660, 40, 690, 40);
		      ((Graphics2D)g).setColor(Color.yellow);
		      ((Graphics2D)g).drawLine(778, 40, 808, 40);
		      ((Graphics2D)g).setColor(Color.green);
		      ((Graphics2D)g).drawLine(895, 40, 925, 40);
	 ...
	 class Chart_ImgPanel extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			for(int i=0;i<list1.size()-1;i++){
				DMIVO temp1=list1.get(i);
				DMIVO temp2=list1.get(i+1);
				double Shoushu1=Double.parseDouble(temp1.getZhengDI());
				double Shoushu2=Double.parseDouble(temp2.getZhengDI());
				int x1=(int)(1+i*18);
			...
			for(int i=0;i<list1.size()-1;i++){
				DMIVO temp1=list1.get(i);
				DMIVO temp2=list1.get(i+1);
				double Shoushu1=Double.parseDouble(temp1.getFuDI());
				double Shoushu2=Double.parseDouble(temp2.getFuDI());
				int x1=(int)(1+i*18);
				int y1=(int)(515-(Shoushu1-datamin)*perXiangSu);
				int x2=(int)(19+i*18);
				int y2=(int)(515-(Shoushu2-datamin)*perXiangSu);
			...
			for(int i=0;i<list1.size()-1;i++){
				DMIVO temp1=list1.get(i);
				DMIVO temp2=list1.get(i+1);
				double Shoushu1=Double.parseDouble(temp1.getADX());
				double Shoushu2=Double.parseDouble(temp2.getADX());
				int x1=(int)(1+i*18);
				int y1=(int)(515-(Shoushu1-datamin)*perXiangSu);
				int x2=(int)(19+i*18);
				int y2=(int)(515-(Shoushu2-datamin)*perXiangSu);
			...
			for(int i=0;i<list1.size()-1;i++){
				DMIVO temp1=list1.get(i);
				DMIVO temp2=list1.get(i+1);
				double Shoushu1=Double.parseDouble(temp1.getADXR());
				double Shoushu2=Double.parseDouble(temp2.getADXR());
				int x1=(int)(1+i*18);
				int y1=(int)(515-(Shoushu1-datamin)*perXiangSu);
				int x2=(int)(19+i*18);
				int y2=(int)(515-(Shoushu2-datamin)*perXiangSu);
	 ...
	 class Chart_LocPanel extends JPanel{
		 protected void paintComponent(Graphics g) {
				super.paintComponent(g);//保留该面板上的其他组件
					...
					g.fillOval(xLoc-5, yLoc4-5, 10, 10);
		 ...
		 Chart_LocPanel(){
				...
				public void mouseClicked(MouseEvent e) {
					...
					DMIVO temp1=list1.get(selected);
					double data1=Double.parseDouble(temp1.getZhengDI());
					
					double data2=Double.parseDouble(temp1.getFuDI());
					
					double data3=Double.parseDouble(temp1.getADX());
					double data4=Double.parseDouble(temp1.getADXR());
						...
						yLoc4=(int)(515-(data4-datamin)*perXiangSu);
					
						DMI_BasePanel.this.setInfo(selected);
	...
	public DMI_BasePanel() {
       ...
       LB3.setBounds(603, 71, 155, 39);
       add(LB3);
       
       JLabel lblbias = new JLabel("趋向指标(DMI)");
       lblbias.setForeground(Color.WHITE);
       lblbias.setFont(new Font("微软雅黑", Font.BOLD, 22));
       lblbias.setBounds(324, 11, 174, 49);
       add(lblbias);
       
       JLabel lblBias = new JLabel("+DI");
       lblBias.setForeground(Color.WHITE);
       lblBias.setFont(new Font("微软雅黑", Font.BOLD, 16));
       lblBias.setBounds(510, 19, 56, 39);
       add(lblBias);
       
       JLabel lblBias_2 = new JLabel("-DI");
       lblBias_2.setForeground(Color.WHITE);
       lblBias_2.setFont(new Font("微软雅黑", Font.BOLD, 16));
       lblBias_2.setBounds(613, 19, 37, 39);
       add(lblBias_2);
       
       JLabel lblBias_1 = new JLabel("ADX");
       lblBias_1.setForeground(Color.WHITE);
       lblBias_1.setFont(new Font("微软雅黑", Font.BOLD, 16));
       lblBias_1.setBounds(721, 20, 50, 39);
       add(lblBias_1);
       ...
       LB4.setFont(new Font("微软雅黑", Font.BOLD, 18));
       LB4.setBounds(762, 71, 186, 39);
       add(LB4);
       
       JLabel lblAdxr_1 = new JLabel("ADXR");
       lblAdxr_1.setForeground(Color.WHITE);
       lblAdxr_1.setFont(new Font("微软雅黑", Font.BOLD, 16));
       lblAdxr_1.setBounds(840, 20, 50, 39);
       add(lblAdxr_1);
	...
	public void DataInit(String numOfStock,String date){
		...
		StocksStatisticsBLService bl=new StocksStatisticsBL();
		
		
		try {
			list1=bl.ComputeDMI(numOfStock, date, 50);
		...
		for(int i=0;i<50;i++){
			DMIVO tempVO1=list1.get(i);
			double Data1=Double.parseDouble(tempVO1.getZhengDI());
			double Data2=Double.parseDouble(tempVO1.getFuDI());
			double Data3=Double.parseDouble(tempVO1.getADX());
			double Data4=Double.parseDouble(tempVO1.getADXR());
			if(i!=0){
	...
	void setInfo(int select){
		...
		DMIVO tempVO1=list1.get(select);
...
//		String Feestr = df.format(Fee);
		...
		LB1.setText("+DI："+tempVO1.getZhengDI());
		LB2.setText("-DI："+tempVO1.getFuDI());
		LB3.setText("ADX："+tempVO1.getADX());
		LB4.setText("ADXR："+tempVO1.getADXR());
	}
	void LocChartReboot(){
		xLoc=2000;
		yLoc1=2000;
