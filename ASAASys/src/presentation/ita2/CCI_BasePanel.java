...
import presentation.ita2.Vr_BasePanel.Chart_ImgPanel;
import presentation.ita2.Vr_BasePanel.Chart_LocPanel;
import vo.CCIVO;
import vo.VrVO;
import businesslogic.StocksBL.StocksBLIndex;
import bussinesslogicservice.StocksBLService.StocksBLIndexService;

public class CCI_BasePanel extends JPanel {
	 ...
	 class Chart_ImgPanel extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			for(int i=0;i<list1.size()-1;i++){
				CCIVO temp1=list1.get(i);
				CCIVO temp2=list1.get(i+1);
				double Shoushu1=Double.parseDouble(temp1.getCCI());
				double Shoushu2=Double.parseDouble(temp2.getCCI());
				int x1=(int)(1+i*18);
	 ...
	 class Chart_LocPanel extends JPanel{
		 ...
		 Chart_LocPanel(){
				...
				public void mouseClicked(MouseEvent e) {
					...
					CCIVO temp1=list1.get(selected);
					double data1=Double.parseDouble(temp1.getCCI());
						...
						CCI_BasePanel.this.setInfo(selected);
	...
	public CCI_BasePanel() {
       ...
       JLabel lblbias = new JLabel("顺势指标(CCI)");
	...
	public void DataInit(String numOfStock,String date){
		...
		StocksBLIndexService bl=new StocksBLIndex();
	      ...
	      try {
			list1=bl.getCCI(numOfStock, 50, date);
		...
		for(int i=0;i<50;i++){
			CCIVO tempVO1=list1.get(i);
			
			double Data1=Double.parseDouble(tempVO1.getCCI());
	...
	void setInfo(int select){
		...
		CCIVO tempVO1=list1.get(select);
...
//		String Feestr = df.format(Fee);
		...
		LB1.setText("CCI："+tempVO1.getCCI());
