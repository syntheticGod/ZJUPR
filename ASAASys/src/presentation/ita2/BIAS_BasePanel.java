...
import presentation.ita2.VolumeChart_BasePanel.Chart_LocPanel;
import presentation.ita2.VolumeChart_BasePanel.FeeChart_ImgPanel;
import presentation.ita2.VolumeChart_BasePanel.ShoushuChart_ImgPanel;
import vo.BIASVO;
import vo.StocksVO;

public class BIAS_BasePanel extends JPanel {
	...
	ArrayList<BIASVO> list1;
	ArrayList<BIASVO> list2;
	ArrayList<BIASVO> list3;
	double datamax;
	double datamin;
	double perXiangSu;
	 ...
	 class Chart_ImgPanel extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			for(int i=0;i<list1.size()-1;i++){
				BIASVO temp1=list1.get(i);
				BIASVO temp2=list1.get(i+1);
			...
			for(int i=0;i<list2.size()-1;i++){
				BIASVO temp1=list2.get(i);
				BIASVO temp2=list2.get(i+1);
				double Shoushu1=Double.parseDouble(temp1.getData());
				double Shoushu2=Double.parseDouble(temp2.getData());
				int x1=(int)(1+i*18);
				int y1=(int)(515-(Shoushu1-datamin)*perXiangSu);
			...
			for(int i=0;i<list3.size()-1;i++){
				BIASVO temp1=list3.get(i);
				BIASVO temp2=list3.get(i+1);
				double Shoushu1=Double.parseDouble(temp1.getData());
				double Shoushu2=Double.parseDouble(temp2.getData());
				int x1=(int)(1+i*18);
				int y1=(int)(515-(Shoushu1-datamin)*perXiangSu);
	 ...
	 class Chart_LocPanel extends JPanel{
		 ...
		 Chart_LocPanel(){
				...
				public void mouseClicked(MouseEvent e) {
					...
					BIASVO temp1=list1.get(selected);
					double data1=Double.parseDouble(temp1.getData());
					BIASVO temp2=list2.get(selected);
					double data2=Double.parseDouble(temp2.getData());
					BIASVO temp3=list3.get(selected);
					double data3=Double.parseDouble(temp3.getData());
						...
						BIAS_BasePanel.this.setInfo(selected);
	...
	public void DataInit(String numOfStock,String date){
		...
		try {
			list1=bl.ComputeBIAS(numOfStock, date, 50, 6);
			list2=bl.ComputeBIAS(numOfStock, date, 50, 12);
			list3=bl.ComputeBIAS(numOfStock, date, 50, 24);
		...
		for(int i=0;i<50;i++){
			BIASVO tempVO1=list1.get(i);
			BIASVO tempVO2=list2.get(i);
			BIASVO tempVO3=list3.get(i);
	...
	void setInfo(int select){
		...
		BIASVO tempVO1=list1.get(select);
		BIASVO tempVO2=list2.get(select);
		BIASVO tempVO3=list3.get(select);
...
//		String Feestr = df.format(Fee);
		...
		LB1.setText("BIAS6："+tempVO1.getData()+"%");
		LB2.setText("BIAS12："+tempVO2.getData()+"%");
		LB3.setText("BIAS24："+tempVO3.getData()+"%");
	}
	void LocChartReboot(){
		xLoc=2000;
		yLoc1=2000;
